...
const getters = {
  ...
  // is this a UCCX demo?
  isUccx: (state, getters) => getters.configuration.demo === 'uccx',
  // is this a PCCE demo?
  isPcce: (state, getters) => getters.configuration.demo === 'pcce',
  ...
  isEce: (state, getters) => getters.configuration.multichannel === 'ece'
}
...
const actions = {
  ...
  async getSessionInfo ({getters, commit, dispatch}, showNotification = true) {
    try {
      ...
      const response = await load(getters.instance, getters.jwt, getters.endpoints.sessionInfo)
